...
Steps 1, 2 and 3 are done. **Step 4 is now the front of the queue.** `composer lint` exits 0 with no errors at all: the six `WordPress.Security.NonceVerification` findings were resolved with items #5 and #6 — ACF verifies the nonce before the `acf/fields/select/query` filter fires, so the sniff is silenced with that justification at the two `$_POST` reads in `Traits\With_Posted_Input`.
...
`npm run lint` and `composer lint` both exit 0. The last six phpcs errors were the nonce findings belonging to audit items #5 and #6, left visible until those items were fixed.
