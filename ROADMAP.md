...
| # | Item | Location | Issue | Status |
|---|---|---|---|---|
| 5 | **Unsanitized `$_POST` into outbound URL path.** `$_POST['taxonomy_selected']` is read with no `isset`/`wp_unslash`/sanitize, then concatenated into the news REST path. Editor-controlled path traversal on the outbound request + transient-key poisoning. Auth is inherited from ACF's AJAX handler, so scope is authenticated editors. Whitelist against `News_Block::ALLOWED_TAX`. *Fixed in [#136](https://github.com/ucsc/ucsc-custom-functionality/pull/136), which rewrote the same handler for [#135](https://github.com/ucsc/ucsc-custom-functionality/issues/135); the allow-list is the taxonomies' REST bases, not `ALLOWED_TAX`'s taxonomy names.* | [News_Blocks_Hooks.php:83](src/Hooks/News_Blocks_Hooks.php#L83), [:122](src/Hooks/News_Blocks_Hooks.php#L122) | [#103](https://github.com/ucsc/ucsc-custom-functionality/issues/103) | ✅ Done |
| 6 | Same unguarded `$_POST` read into `get_terms()`. Lower impact (WP validates taxonomy) but same missing guards. | [Taxonomies_Hooks.php:89](src/Hooks/Taxonomies_Hooks.php#L89) | [#103](https://github.com/ucsc/ucsc-custom-functionality/issues/103) | ✅ Done |
| 7 | **Fatal risk:** `in_array( 'embed-post', $query['slug__in'] )` — index unguarded; `get_block_templates` callers frequently omit `slug__in` → `TypeError` on PHP 8. | [Post_Single.php:16](src/Template/Post_Single.php#L16) | [#104](https://github.com/ucsc/ucsc-custom-functionality/issues/104) | ✅ Done |
| 8 | **Fatal risk:** `get_current_screen()` may return `null`; `->base` dereferenced unguarded. | [plugin.php:60](plugin.php#L60) | [#104](https://github.com/ucsc/ucsc-custom-functionality/issues/104) | ✅ Done |
| 9 | Unguarded index `$this->query_loop[ MANUAL_CARDS ]` in manual query mode. | [Query_Loop_Controller.php](src/Components/Query_Loop_Controller.php) | [#104](https://github.com/ucsc/ucsc-custom-functionality/issues/104) | ✅ Done |
...
| 24 | No test suite. Nothing enforces the block-registration path contract that broke #1 — a smoke test asserting every `Core::BLOCKS_*` path resolves to a real `block.json` would have caught it. *A PHPUnit + Brain Monkey suite now exists (`composer test`), added with #5–#9; it covers only those fixes. The registration smoke test is still to write.* | — | [#111](https://github.com/ucsc/ucsc-custom-functionality/issues/111) | Open |
...
| Now | 0 errors / 0 warnings | 0 | clean |

`npm run lint` and `composer lint` both exit 0. The last six phpcs errors were the nonce findings belonging to audit items #5 and #6, left visible until those items were fixed — #5 on `main` via [#136](https://github.com/ucsc/ucsc-custom-functionality/pull/136), #6 with the rest of step 3.
